...
//	int snak_y = 2;
	int sneksize = 20;
	bool key[8]{ false, true, false, false, true, false, false, false };
...
//	al_reserve_samples(10);
	...
	/*		if (Eatgoal == true) {
			...
			if (key[4] && P2head_y > 0)
				P2head_y -= 1;
			if (key[5] && P2head_y < 40)
				P2head_y += 1;
			if (key[6] && P2head_x < 0)
				P2head_x -= 1;
			if (key[7] && P2head_x < 40)
				P2head_x += 1;
...
//				al_play_sample(bite, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE, NULL);
				...
				al_draw_textf(font, al_map_rgb(255, 100, 100), 300, 310, NULL, "Tron2 done winned dude");
				al_flip_display();
				al_rest(2);
				return 0;
			}
